...
public class CustomLogger {
    ...
    private static final List<String> SENSITIVE_FIELDS = Arrays.asList("password", "token", "secret", "authorization");
    ...
    public void bodyError(Exception e, String errorCode, String errorMessage, int statusCode, HttpServletRequest request) {
        if (shouldLog("error")) {
            String logMessage = String.format(
                ...
                "%s* STACK TRACE:   %s%s\n" +
                "%s━━━━━━━━━━━━━━━━",
                ANSI_RED, new java.util.Date().toString(), ANSI_RESET,
    ...
    private String sanitizeSensitiveData(String body) {
        List<String> sensitiveFields = Arrays.asList("password", "token", "secret", "authorization");
        for (String field : sensitiveFields) {
            body = body.replaceAll("(?i)\"" + field + "\":\"[^\"]*\"", "\"" + field + "\":\"[REDACTED]\"");
        }
        return body;
    }
    ...
    private boolean isSensitiveField(String fieldName) {
        return SENSITIVE_FIELDS.stream()
                .anyMatch(field -> fieldName.toLowerCase().contains(field));
    }
} 