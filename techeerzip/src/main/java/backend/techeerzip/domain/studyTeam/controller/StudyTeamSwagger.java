...
public interface StudyTeamSwagger {
    ...
    ResponseEntity<Long> createStudyTeam(
            @Parameter(description = "스터디 결과 이미지 목록", content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE))
            @RequestPart(value = "resultImages", required = false)
            List<MultipartFile> resultImages,

            @Parameter(description = "스터디 생성 요청 DTO", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE))
