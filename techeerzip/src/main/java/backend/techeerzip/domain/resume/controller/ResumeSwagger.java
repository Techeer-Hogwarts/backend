...
public interface ResumeSwagger {
    ...
    default ResponseEntity<ResumeCreateResponse> createResume(
            ...
            @RequestPart("file") @Parameter(description = "업로드할 이력서 파일") MultipartFile file,
            @RequestPart("request") @Parameter(description = "이력서 정보")
                    ResumeCreateRequest request) {
        throw new UnsupportedOperationException("Swagger 전용 인터페이스입니다.");
    }

    @Operation(summary = "이력서 단일 조회", description = "이력서 ID로 단일 이력서를 조회합니다.")
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
    ...
    @Operation(summary = "이력서 삭제", description = "단일 이력서를 삭제합니다.")
    @ApiResponses({
    ...
    @Operation(summary = "메인 이력서 지정", description = "특정 이력서를 메인 이력서로 지정합니다. 기존 메인 이력서는 자동으로 해제됩니다.")
    @ApiResponses({
    ...
    @Operation(
            summary = "이력서 목록 조회",
            description =
                    "여러 조건(직책, 기수, 카테고리 등)으로 이력서 목록을 조회합니다.\n\n"
    ...
    default ResponseEntity<ResumeListResponse> getResumes(
            ...
            @Parameter(description = "검색할 기수 (여러 개 가능)", example = "[1, 2, 3]", required = false)
                    List<Integer> year,
            @Parameter(
                            ...
                            required = false)
                    String category,
            @Parameter(description = "커서 ID", example = "0", required = false) Long cursorId,
            @Parameter(description = "가져올 개수 (기본값: 10)", example = "10", required = false)
                    Integer limit) {
        throw new UnsupportedOperationException("Swagger 전용 인터페이스입니다.");
    }
    ...
    @Operation(
            ...
            description =
                    "최근 2주 이내 생성된 이력서 중 인기(계산식: viewCount + likeCount * 10)순으로 정렬된 이력서 목록을 조회합니다.")
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
                description = "인기 이력서 목록 조회 성공",
                content = @Content(schema = @Schema(implementation = ResumeListResponse.class)))
    })
