...
/**
 * 프로젝트 팀을 커서 기반으로 조회하기 위한 요청 DTO입니다.
 * 정렬 기준에 따라 커서 필드를 함께 사용하며, 포지션 조건과 모집 상태로 필터링 가능합니다.
 *
 * <p>서비스에서는 {@code sortType}에 따라 커서 기준 필드가 달라지며,
 * {@code idCursor}, {@code dateCursor}, {@code countCursor}가 해당 정렬에 맞춰 사용됩니다.</p>
 *
 * <ul>
 *   <li>{@code sortType}이 {@code UPDATE_AT_DESC}인 경우 → {@code dateCursor}, {@code idCursor}를 커서 기준으로 사용</li>
 *   <li>{@code sortType}이 {@code VIEW_COUNT_DESC} 또는 {@code LIKE_COUNT_DESC}인 경우 → {@code countCursor}, {@code idCursor}를 커서 기준으로 사용</li>
 * </ul>
 *
 * <p>널일 경우의 동작:</p>
 * <ul>
 *   <li>{@code idCursor} → null이면 ID 비교 없이 단일 커서 필드만으로 조회</li>
 *   <li>{@code dateCursor}, {@code countCursor} → null이면 커서 조건 없이 최신부터 정렬</li>
 *   <li>{@code positionNumTypes} → null 또는 빈 리스트이면 포지션 조건 없이 전체 포함</li>
