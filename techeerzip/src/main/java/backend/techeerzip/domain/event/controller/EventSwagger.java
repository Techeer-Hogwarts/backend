...
public interface EventSwagger {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "생성 성공", content = @Content(schema = @Schema(implementation = EventCreateResponse.class))),
    ...
    default ResponseEntity<EventCreateResponse> updateEvent(
            ...
            @RequestBody(description = "이벤트 수정 요청 객체", required = true,
                    content = @Content(schema = @Schema(implementation = EventCreateRequest.class)))
            EventCreateRequest request,
