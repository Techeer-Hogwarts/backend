...
public class SessionDSLRepositoryImpl implements SessionDSLRepository {
     ...
     *
     * <p>커서 조건 (createdAt, id 기준): - createdAt이 더 과거 - 같다면 id가 더 작음
     *
     * <p>정렬 기준 (최신순): - createdAt 내림차순 - id 내림차순
     */
    @Override
    public SessionListResponse<Session> findAllByCursor(SessionListQueryRequest request) {
        ...
        BooleanExpression cursorCondition =
                buildCreatedAtCursorCondition(
                        filterCondition, request.cursor(), request.createdAt());
     ...
     *
     * <p>커서 조건 (viewCount, createdAt, id 우선순위로 비교): - viewCount가 더 작음 - 같다면 createdAt이 더 과거 - 같다면
     * id가 더 작음
     *
     * <p>정렬 기준 (인기순): - viewCount 내림차순 - createdAt 내림차순 - id 내림차순
     */
    @Override
    public SessionBestListResponse<Session> findAllBestSessionsByCursor(
        ...
        if (request.cursor() != null
            ...
            cursorCondition =
                    session.viewCount
                            .lt(request.viewCount())
                            .or(
                                    session.viewCount
                                            .eq(request.viewCount())
                                            .and(
                                                    session.createdAt
                                                            .lt(request.createdAt())
                                                            .or(
                                                                    session.createdAt
                                                                            .eq(request.createdAt())
                                                                            .and(
                                                                                    session.id.lt(
                                                                                            request
                                                                                                    .cursor())))));
        ...
        List<Session> results =
                queryFactory
                        .selectFrom(session)
                        .where(session.createdAt.goe(twoWeeksAgo), cursorCondition)
                        .orderBy(
    ...
    private BooleanExpression addCondition(
            BooleanExpression existing, BooleanExpression newCondition) {
