...
public class LikeService {
                                ...
                                .ifPresentOrElse(
                                                like -> {
                                                        if (like.isDeleted()) {
                                                                ...
                                                                updateContentLikeCount(request.getContentId(),
                                                                                LikeCategory.valueOf(
                                                                                                request.getCategory()),
                                                                                true);
                                                                logger.info("삭제된 좋아요 재활성화 처리 완료 | context: {}",
                                                                                CONTEXT);
                                                        } else {
                                                ...
                                                () -> {
                                                        Like newLike = new Like(request.getContentId(),
        ...
        private void updateContentLikeCount(Long contentId, LikeCategory category, boolean isLiked) {
                switch (category) {
                        case BLOG -> {
                                Blog blog = blogRepository.findByIdAndIsDeletedFalse(contentId)
                                ...
                                logger.info("블로그 좋아요 카운트 업데이트 완료 - blogId: {}, isLiked: {} | context: {}",
                                                contentId, isLiked, CONTEXT);
                        }
                        case SESSION -> {
                                Session session = sessionRepository.findByIdAndIsDeletedFalse(contentId)
                                ...
                                logger.info("세션 좋아요 카운트 업데이트 완료 - sessionId: {}, isLiked: {} | context: {}",
                                                contentId, isLiked, CONTEXT);
                        }
                        case RESUME -> {
                                Resume resume = resumeRepository.findByIdAndIsDeletedFalse(contentId)
                                ...
                                logger.info("이력서 좋아요 카운트 업데이트 완료 - resumeId: {}, isLiked: {} | context: {}",
                                                contentId, isLiked, CONTEXT);
                        }
                }
        }
