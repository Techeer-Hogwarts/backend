...
public class LikeRepositoryImpl extends QuerydslRepositorySupport implements LikeRepositoryCustom {
    ...
    public List<Like> findLikesWithCursor(Long userId, String category, Long cursorId, int limit) {
        return queryFactory
                .selectFrom(like)
                .leftJoin(blog).on(like.category.eq("BLOG").and(like.contentId.eq(blog.id)))
                .leftJoin(session).on(like.category.eq("SESSION").and(like.contentId.eq(session.id)))
                .leftJoin(resume).on(like.category.eq("RESUME").and(like.contentId.eq(resume.id)))
                .where(
