...
public interface TechBloggingChallengeSwagger {
        ...
        @Operation(summary = "회차별 블로그 조회", description = """
                        ...
                        """, responses = {
                        @ApiResponse(responseCode = "200", description = "블로그 목록 조회 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = BlogChallengeListResponse.class), examples = {
                                        @ExampleObject(name = "최신순 조회 예시", value = """
                                                        {
                                                            "blogs": [
                                                                {
                                                                    "id": 1,
                        ...
                        @ApiResponse(responseCode = "400", description = "잘못된 요청", content = @Content(mediaType = "application/json", examples = {
                                        @ExampleObject(name = "회차 ID만 있는 경우", value = """
                                                        {
                                                            "message": "회차 ID만으로는 조회할 수 없습니다. 챌린지 기간(termId)을 함께 입력해주세요."
                                                        }
                                                        """),
                                        @ExampleObject(name = "잘못된 회차 ID", value = """
                                                        {
        ...
        ResponseEntity<BlogChallengeListResponse> getBlogsByRoundCursor(
                        @Parameter(description = "챌린지 기간 ID (없으면 현재 진행중인 챌린지)", example = "1") @RequestParam(required = false) Long termId,
                        @Parameter(description = "회차 ID (termId와 함께 사용)", example = "1") @RequestParam(required = false) Long roundId,
                        @Parameter(description = "커서 ID (이전 조회의 마지막 블로그 ID)", example = "100") @RequestParam(required = false) Long cursorId,
                        @Parameter(description = "조회할 개수", example = "10") @RequestParam(required = false, defaultValue = "10") int limit,
                        @Parameter(description = "정렬 기준 (latest/viewCount/name)", example = "latest") @RequestParam(required = false, defaultValue = "latest") String sortBy);
