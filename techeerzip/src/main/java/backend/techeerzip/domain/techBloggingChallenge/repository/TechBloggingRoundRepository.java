...
public interface TechBloggingRoundRepository extends JpaRepository<TechBloggingRound, Long> {
    ...
    @Query(
            "SELECT r FROM TechBloggingRound r WHERE r.term.firstHalf = :firstHalf AND r.startDate BETWEEN :startDate AND :endDate AND r.isDeleted = false")
    List<TechBloggingRound> findByTermFirstHalfAndStartDateBetween(
            @Param("firstHalf") boolean firstHalf,
