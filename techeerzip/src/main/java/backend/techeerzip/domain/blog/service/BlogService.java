...
public class BlogService {
        ...
        public BlogListResponse getBestBlogs(BlogBestQueryRequest query) {
                logger.info(
                                ...
                                query.limit(),
                                CONTEXT);
                List<Blog> blogs = blogRepository.findPopularBlogsWithCursor(query.cursorId(), query.limit());
        ...
        public BlogResponse getBlog(Long blogId) {
                ...
                Blog blog = blogRepository
                                ...
                                .orElseThrow(
                                                () -> {
                                                        logger.warn(
                                                                        String.format("블로그 조회 실패 - 존재하지 않는 blogId: %d",
                                                                                        blogId),
                                                                        CONTEXT);
                ...
                Blog blog = blogRepository
                                ...
                                .orElseThrow(
                                                () -> {
                                                        logger.warn(
                                                                        String.format(
                                                                                        "블로그 조회수 증가 실패 - 존재하지 않는 blogId: %d",
                                                                                        blogId),
                                                                        CONTEXT);
                                                        return new BlogNotFoundException();
                                                });
        ...
        public BlogResponse deleteBlog(Long blogId, Long currentUserId) {
                ...
                if (!blog.getUser().getId().equals(currentUserId)) {
                        logger.warn(
                                        String.format(
                                                        "블로그 삭제 실패 - 권한 없음 - blogId: %d, userId: %d", blogId,
                                                        currentUserId),
                                        CONTEXT);
