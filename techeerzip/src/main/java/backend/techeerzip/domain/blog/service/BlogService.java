...
public class BlogService {
        ...
        public List<BlogResponse> getBlogList(BlogsQueryRequest query) {
                logger.debug(
                                String.format(
                                                "query - category: %s, offset: %d, limit: %d",
                                                query.getCategory(), query.getOffset(), query.getLimit()),
                                CONTEXT);

                Pageable pageable = PageRequest.of(query.getOffset(), query.getLimit());
                List<Blog> blogs = query.getCategory() != null
                                ? blogRepository
                                                .findByIsDeletedFalseAndCategoryOrderByCreatedAtDesc(
                                                                query.getCategory(), pageable)
                                                .getContent()
                                : blogRepository.findAll(pageable).getContent();

                logger.debug(String.format("%d개의 블로그 엔티티 목록 조회 성공", blogs.size()), CONTEXT);
                return blogs.stream().map(BlogResponse::new).collect(Collectors.toList());
        }
        ...
        public List<BlogResponse> getBlogsByUser(Long userId, int page, int size) {
                ...
                List<Blog> blogs = blogRepository
                                .findByIsDeletedFalseAndUserIdOrderByCreatedAtDesc(userId, pageable)
                                .getContent();
                                ...
                                .forEach(
                                                post -> {
                                                        try {
                                                                ...
                                                                try {
                                                                        postDate = LocalDateTime.parse(post.getDate(),
                                                                                        ISO_DATE_TIME);
