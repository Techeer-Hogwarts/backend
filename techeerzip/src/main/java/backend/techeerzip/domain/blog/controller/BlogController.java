...
public class BlogController {
    ...
    @PutMapping("/{blogId}")
    public ResponseEntity<Void> increaseBlogViewCount(
    ...
    @Operation(summary = "블로그 게시물의 인기글 목록 조회",
    ...
    public ResponseEntity<List<BlogResponse>> getBlogsByUser(
            ...
            @Parameter(description = "페이지네이션 정보") @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "페이지 크기") @RequestParam(defaultValue = "10") int size) {
        logger.info("유저 별 블로그 게시물 목록 조회 처리 중 - userId: %d, page: %d, size: %d" + userId + page + size + CONTEXT);
